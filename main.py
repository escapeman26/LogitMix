...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not load_networks:
        ...
        record_best('./Summary/{}'.format(args.network), args, best_acc1)

        acc_txt_dir = './Summary/{}'.format(args.network) + '/acc_only.txt'
        if not os.path.exists(acc_txt_dir):
            with open(acc_txt_dir, 'w') as file:
                file.write('{0:}| {1:}| best_acc: {2:} \n'
                           .format(args.time, args.mixmethod, best_acc1))
        else:
            with open(acc_txt_dir, 'a') as file:
                file.write('{0:}| {1:}| best_acc: {2:} \n'
                           .format(args.time, args.mixmethod, best_acc1))
